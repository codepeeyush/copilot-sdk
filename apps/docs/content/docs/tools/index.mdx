...
<Cards>
  <Card title="useToolWithSchema" icon="✨">
    Recommended. Type-safe with Zod schemas.
  </Card>
  <Card title="useToolsWithSchema" icon="📦">
    Register multiple Zod-based tools at once.
  </Card>
  <Card title="useTool" icon="🔧">
    Basic tool with JSON Schema. Less type-safe.
  </Card>
  <Card title="useTools" icon="📋">
    Register multiple basic tools at once.
  </Card>
</Cards>
...
## Multiple Tools with useToolsWithSchema
...
## Basic Tool with useTool

For simpler cases without Zod:
...
Use `useToolExecutor` to track and manage tool executions:
...
function ToolStatusPanel() {
  const {
    executions,       // All tool executions
    pendingCount,     // Waiting for approval
    runningCount,     // Currently executing
    completedCount,   // Finished
    approve,          // Approve a pending tool
    reject,           // Reject a pending tool
  } = useToolExecutor();
  ...
  return (
    <div>
      ...
      {executions.map(exec => (
        <div key={exec.id} className="tool-execution">
...
```tsx
import { createPermissionStorage } from '@yourgpt/copilot-sdk-react';

// Create persistent storage
const permissionStorage = createPermissionStorage({
  type: 'localStorage',
...
type PermissionLevel =
  ...
  | 'allow_always'    // Remember and always allow
  | 'deny_once'       // Deny this one time
  | 'deny_always';    // Remember and always deny
```
...
```tsx
import { success, failure } from '@yourgpt/copilot-sdk-core';

// Success helper
return success({ products: results }, 'Found 5 products');

// Failure helper
return failure('Product not found', { searched_id: productId });
```
...
## Defining Tools with Core Utilities

For advanced use cases, use core utilities:

```tsx
import { defineTool, defineClientTool, defineServerTool } from '@yourgpt/copilot-sdk-core';
import { z } from 'zod';
...
const clientTool = defineClientTool({
  name: 'scroll_to',
...
// Server-side tool (runs on server)
const serverTool = defineServerTool({
  name: 'query_database',
  description: 'Query the database',
  schema: z.object({ sql: z.string() }),
  // No handler - handled server-side
});
```
